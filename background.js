/*!
 * Bilibili 观看记录增强 —— 后台 Service Worker
 *
 * 职责：
 *  1. 调用 B 站官方 API 获取视频信息（标题 / UP主），API 优先，失败时回退页面 DOM 数据；
 *  2. 本地存储观看记录（仅标题 / 源链接 / 发布者 / 最后观看时间，精确到小时），按链接去重；
 *  3. 提供搜索、导出 JSON、删除、清空，以及从官方观看历史 API 同步（需登录）；
 *  4. 设置管理（颜色主题 / 背景图片 / 自动同步），自动同步通过 chrome.alarms 定时触发；
 *  5. 多等级日志（debug / warn / error），保存在本地，供设置页日志查看器排查问题；
 *  6. 官方历史接口优先使用 WBI 签名（wbi_sign + wts），失败时回退无签名请求并记录日志。
 */
'use strict';
